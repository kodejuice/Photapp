...
const VideoViewer: React.FC<{url:string}> = ({url})=>{
    ...
    return (
        <React.Fragment>
            ...
            <div style={{height: '100%'}} onClick={()=>setPlaying(!isPlaying)}>
                <ReactPlayer {...config} />
            </div>
 ...
 * @param  {string[]}          mentions    mentioned users usernames
 * @param  {'image'|'video'[]} media_types media types of each url
 */
const MediaViewer: React.FC<ViewerProp> = ({paths, mentions, media_types})=>{
    ...
    const onArrowClick = (click)=>{
        // dirty hack that i'm ashamed of :(
        // stop all videos in this component from playing when we move to another slide
